...
LAST_CLOSE_FILE = "last_window_close.txt"
...
def initialize_files():
    ...
    if not os.path.exists(LAST_NOTIFICATION_FILE):
        with open(LAST_NOTIFICATION_FILE, 'w') as f:
            f.write("")
    if not os.path.exists(LAST_CLOSE_FILE):
        with open(LAST_CLOSE_FILE, 'w') as f:
            f.write("")

def get_last_close_time():
    try:
        with open(LAST_CLOSE_FILE, 'r') as f:
            content = f.read().strip()
            if content:
                username, timestamp_str = content.split(',')
...
def save_close_time():
    ...
    with open(LAST_CLOSE_FILE, 'w') as f:
        f.write(f"{username},{current_time}")

def check_should_show_window():
    ...
    # Check if it's been 3 hours since last close
    return (current_time - last_close) >= three_hours_in_seconds
...
def has_mood_saved_today():
    ...
    try:
        with open(MOOD_FILE, 'r') as f:
            ...
            for row in reader:
                if len(row) >= 2:
                    timestamp, user = row[0:2]
                    entry_date = datetime.strptime(timestamp, "%Y-%m-%d %H:%M:%S").date().isoformat()
                    if user == username and entry_date == today:
                        return True
...
class MoodWindow(tk.Tk):
    ...
    def on_closing(self):
        if not has_mood_saved_today():
            save_close_time()
        self.destroy()

def show_notification():
    if check_should_show_window() and not has_mood_saved_today():
        root = MoodWindow()
        root.mainloop()
...
def schedule_next_check():
    if not has_mood_saved_today():
        ...
        if last_close is not None:
            ...
            if time_since_close < three_hours_in_seconds:
                # Wait for the remaining time and then show the window
                time_to_wait = three_hours_in_seconds - time_since_close
